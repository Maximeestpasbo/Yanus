...
public class Nuke extends BombObject implements Demisable{
	...
	public void run() {
		...
		this.demisableNotifyObserver(); // fait l'explosion et enlève la bombe
		...
		for(int i = x-range; i <= x+range; i++){
			for(int j = y-range; j <= y+range; j++){
				...
				for(DemisableObserver observer : demisableObservers){
					explosion.demisableAttach(observer); //enlève l'explosion
				}
				loot.add(explosion); //affiche explosion
			}
		}
